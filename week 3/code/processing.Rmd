...
dat_age <- data_demographics_raw |>
  ...
  filter(item == "age") |>
  rename(age = response) |> #new name = old name
...
dat_gender <- data_demographics_raw |>
  ...
  mutate(gender = str_to_lower(gender),
         ...
         gender = na_if(gender, ""),
  #select(-item) # alternative: negative selection
  #mutate(gender = replace(gender, !(gender %in% c("men", "woman", "male", "female",
                                      # "non-binary", "non binary")), NA_character_)) 
...
dat_gender |>
  distinct(gender) #um alle responses zu sehen
...
data_age_gender_temp <- 
  bind_cols(dat_age, dat_gender) #just puts both next to each other but if the id is in a different order then you're f****

data_age_gender <- 
  full_join(dat_age,
            dat_gender,
            by = "unique_id") #correctly joins the rows to the right id 
```
